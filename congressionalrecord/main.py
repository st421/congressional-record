...
import logging   
        ...
        self.base_path = base_input_path
    ...
    def download_and_write_day(self, day, outpath="output"):
        ...
        if year not in os.listdir(outpath):
            os.mkdir(os.path.join(outpath, year))
        filename = os.path.join(outpath,year, '{0}-{1}.{2}'.format(self.output_prefix, day, self.output_format))
        self.download_and_write(route, filename)   


class BulkCRDownloader(object):
    def __init__(self, **kwargs):
        """
        Invoke a Downloader object to get data from
        the Record. Checks to see if the necessary
        files are already downloaded and use those instead of
        querying an API. Downloaders are the endpoint for raw data.
        ...
        """
        self.status = 'idle'
        self.outpath = kwargs.get("outpath", 'output')

    """
    Chunks through downloads.

    start : In form 'YYYY-MM-DD.' This is the day/start day you want.

    end : Same form as start. This is the end date.
    """
    def download(self, start_day, end_day, **kwargs):
...
class BulkCRDownloader(object):
    def __init__(self, do_mode, **kwargs):
        """
        Invoke a Downloader object to get data from
        the Record. Checks to see if the necessary
        files are already downloaded and use those instead of
        ...
        extract : Defaults to True. Whether or not to extract the downloaded content.
    ...
    """
    def download(self,start,parse=True,**kwargs):
